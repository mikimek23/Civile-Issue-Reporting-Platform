    ...
    const handleSubmit = async (e) => {
        ...
        try {
          console.log("Submitting data:", data);
            const response = await axios.post('http://localhost:5000/report', data, {
                headers: {
      ...
      <div className="w-full bg-white rounded-lg shadow  md:mt-0 sm:max-w-md xl:p-0 ">
          ...
          {message}
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                  <label htmlFor='title' className="block mb-2 text-sm font-medium text-gray-900 mt-4 ">title</label>
                <input type='text' id="title" name="title" value={data.title} placeholder='title' onChange={handleChange} className="bg-gray-50 bor++der border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " required/> 
                ...
                <input type='file' id="photourl" name="title" value={data.photourl} accept='.jpg, .JPG' onChange={handleChange} className="bg-gray-50 bor++der border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " required /> 
